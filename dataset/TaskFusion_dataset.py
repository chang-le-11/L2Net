...
class Fusion_dataset(Dataset):
    def __init__(self, split, ir_path=None, vi_path=None):
        ...
        assert split in ['train', 'train_gray', 'val', 'test', 'val_gray'], 'split must be "train"|"val"|"test"'

        if split == 'train':
            data_dir_vis = 'E:\\my_methods\\train_data\\vis'
            data_dir_ir = 'E:\\my_methods\\train_data\\ir'
            self.filepath_vis, self.filenames_vis = prepare_data_path(data_dir_vis)
            self.filepath_ir, self.filenames_ir = prepare_data_path(data_dir_ir)
            self.split = split
            self.length = min(len(self.filenames_vis), len(self.filenames_ir))

        elif split == 'train_gray':
            data_dir_vis = 'D:\\use\\vis'
            data_dir_ir = 'D:\\use\\ir'
            self.filepath_vis, self.filenames_vis = prepare_data_path(data_dir_vis)
            self.filepath_ir, self.filenames_ir = prepare_data_path(data_dir_ir)
            self.split = split
            self.length = min(len(self.filenames_vis), len(self.filenames_ir))
